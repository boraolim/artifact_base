...
public static class Functions
{
    ...
    private static string GetExceptionDetails(Exception exception)
    {
        ...
        var moduleName = frame?.GetMethod()?.DeclaringType?.Name ?? "Unknown";
        var methodName = $"{method?.DeclaringType?.FullName.Trim()}.{method?.Name.Trim()}" ?? "UnknownMethod";
        ...
        return string.Format(FormatConstantsCore.FMT_FORMAT_EXCEPTION_FULL,
            (exception.InnerException.CheckIsNull()) ? ClearText(exception.Message) : string.Format(FormatConstantsCore.FMT_LEGEND_ERROR,
                ClearText(exception.Message), ClearText(exception.InnerException.Message)), moduleName, methodName,
            frame?.GetFileLineNumber() > 0 ? $" at line {frame.GetFileLineNumber()}" : string.Empty, ClearText(exception.StackTrace));
    }
